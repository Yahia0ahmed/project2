...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        await self.highrise.walk_to([8,0,-1],'FrontRight')

        pass
    async def on_user_join(self, user: User) -> None:
        await self.highrise.chat(f"نورتي الروم يا روح يحيى❤ {user.username}!")
    async def on_user_join(self, user: User) -> None:
        wait self.highrise.chat(f"نورتي الروم يا روح يحيى❤ {user.username}!")
    async def on_tip(self, sender: User, receiver: User, tip: int) -> None:
        await self.highrise.on_tip({sender: User} شكرا لتبرعك الى {receiver: User} بتبرعك {tip: int})
