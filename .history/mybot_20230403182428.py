...
from highrise.models import SessionMetadata, User, Item, GetWalletRequest, Error, Position
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        await self.highrise.walk_to(Position(6,2.5,0,"FrontRight"))
        pass
    async def on_user_join(self, user: User) -> None:
        await self.highrise.chat(chatRequest(f"نورت مزاد النخبة 🔥 {user.username}!")
