...
class Bot(BaseBot):
    ...
    async def on_user_join(self, user: User) -> None:
        await self.highrise.chat(f"نورت الروم يا روح يحيى❤ {user.username}!")
    username_to_id = {}
        ...
        if message.startswith("t"):
            ...
            if len(parts) > 3:
                ...
                try:
                    ...
                    await self.highrise.chat("تنسيق احداثيات غير صحس.")
                    return
                user_id = user_id
            await self.highrise.teleport(user_id, (x, y, z))
            await self.highrise.chat(f"{user.username} تم نقله اللى ({x}, {y}, {z})")
