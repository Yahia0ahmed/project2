    ...
    async def on_chat(self, user: User, message: str) -> None:    
        if message.startswith("‎"):
            ...
            if len(parts) > 3:
                ...
                await self.highrise.chat("Invalid teleport command format.")
                return
           wait self.highrise.teleport(user_id, Position(x, y, z)) a
    async def on_wishper(self, user: User, message: str) -> None:
        if message.startswith('1st'):
            await self.highrise.teleport(user.id,Position(8,0.25,8))
            await self.highrise.send_whisper(f"{user.username} has been teleported to ({x}, {y}, {z})")
            pass
    async def on_whisper(self, user: User, message: str) -> None:
        if message.startswith('2nd'):
            await self.highrise.teleport(user.id,Position(6,0,5))